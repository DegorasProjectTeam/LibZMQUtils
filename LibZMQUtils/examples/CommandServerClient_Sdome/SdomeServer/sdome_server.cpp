...
/** ********************************************************************************************************************
 * @file sdome_server.cpp
 * @brief EXAMPLE FILE - This file contains the implementation of the SdomeServer example class.
...
SdomeServer::SdomeServer(unsigned int port, const std::string &local_addr) :
    ClbkCommandServerBase(port, local_addr)
{
    ...
    this->registerRequestProcFunc(SdomeServerCommand::REQ_SET_HOME_POSITION,
                                  &SdomeServer::processSetHomePosition);

    // REQ_GET_HOME_POSITION.
    this->registerRequestProcFunc(SdomeServerCommand::REQ_GET_HOME_POSITION,
                                  &SdomeServer::processGetHomePosition);
}

void SdomeServer::processSetHomePosition(const CommandRequest& request, CommandReply& reply)
{
...
void SdomeServer::processGetHomePosition(const CommandRequest& request, CommandReply &reply)
{
    // Auxiliar variables and containers.
    controller::ControllerError ctrl_err;
...
void SdomeServer::registerRequestProcFunc(SdomeServerCommand command, SdomeRequestProcFunc func)
{
    CommandServerBase::registerRequestProcFunc(static_cast<ServerCommand>(command), this, func);
}

bool SdomeServer::validateCustomCommand(ServerCommand command)
{
    // Auxiliar variables.
    bool result = false;
...
void SdomeServer::onCustomCommandReceived(const CommandRequest& request, CommandReply& reply)
{
    // Get the command string.
    std::string cmd_str;
    std::uint32_t cmd_uint = static_cast<std::uint32_t>(request.command);
    cmd_str = (cmd_uint < SdomeServerCommandStr.size()) ? SdomeServerCommandStr[cmd_uint] : "Unknown command";
...
void SdomeServer::onServerStart()
{
    // Ips.
    std::string ips;
...
void SdomeServer::onWaitingCommand()
{
    // Log.
    std::cout << std::string(100, '-') << std::endl;
    std::cout<<"<SDOME SERVER>"<<std::endl;
...
void SdomeServer::onDeadClient(const HostInfo& client)
{
    // Log.
    std::cout << std::string(100, '-') << std::endl;
    std::cout<<"<SDOME SERVER>"<<std::endl;
...
void SdomeServer::onConnected(const HostInfo& client)
{
    // Log.
    std::cout << std::string(100, '-') << std::endl;
    std::cout<<"<SDOME SERVER>"<<std::endl;
...
void SdomeServer::onDisconnected(const HostInfo& client)
{
    // Log.
    std::cout << std::string(100, '-') << std::endl;
    std::cout<<"<SDOME SERVER>"<<std::endl;
...
void SdomeServer::onServerError(const zmq::error_t &error, const std::string &ext_info)
{
    // Log.
    std::cout << std::string(100, '-') << std::endl;
    std::cout<<"<SDOME SERVER>"<<std::endl;
...
void SdomeServer::onCommandReceived(const CommandRequest &request)
{
    // Get the command string.
    std::string cmd_str;
    std::uint32_t command = static_cast<std::uint32_t>(request.command);
    cmd_str = (command < SdomeServerCommandStr.size()) ? SdomeServerCommandStr[command] : "Unknown command";
    // Log.
...
void SdomeServer::onInvalidMsgReceived(const CommandRequest &request)
{
    // Log.
    BinarySerializer serializer(request.params.get(), request.params_size);
    std::cout << std::string(100, '-') << std::endl;
...
void SdomeServer::onSendingResponse(const CommandReply &reply)
{
    // Log.
    ...
    std::cout<<"Time: "<<zmqutils::utils::currentISO8601Date()<<std::endl;
    std::cout<<"Result: "<<result<<" ("<<SdomeServerResultStr[result]<<")"<<std::endl;
